...
export const Card: React.FC<{ children: React.ReactNode; className?: string; onClick?: () => void }> = ({ 
  ...
  return (
    <div 
      onClick={onClick}
      className={`bg-white rounded-3xl shadow-soft border border-gray-300 p-5 ${className} ${onClick ? 'cursor-pointer active:translate-y-[2px] active:shadow-none transition-all' : ''}`}
    >
      {children}
    </div>
  );
...
export const Button: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' | 'danger' | 'ghost' }> = ({ 
  ...
  const variants = {
    primary: "bg-brand text-white shadow-soft border-2 border-brand hover:-translate-y-0.5",
    secondary: "bg-white text-ink border-2 border-gray-300 shadow-soft hover:-translate-y-0.5",
...
export const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label?: string }> = ({ 
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      ...
      <input 
        className={`bg-white border-2 border-gray-300 rounded-2xl px-5 py-3 focus:outline-none focus:border-brand focus:ring-4 focus:ring-brand/10 transition-all text-lg shadow-sm placeholder:text-gray-300 ${className}`} 
        {...props} 
      />
    </div>
  );
...
export const TopBar: React.FC<{ title: string; onBack?: () => void; rightAction?: React.ReactNode }> = ({ title, onBack, rightAction }) => (
  <div className="fixed top-0 left-0 right-0 z-50 bg-[#F5FFFA]/90 backdrop-blur-md border-b border-gray-300 pt-[env(safe-area-inset-top)] transition-all duration-300">
