...
import { AlertTriangle, AlertOctagon, Info, Snowflake, CloudRain, Wind, Mountain, CloudLightning, ExternalLink, Megaphone, CheckCircle2 } from 'lucide-react';
...
export const WeatherJMAWarningCard: React.FC<WeatherJMAWarningCardProps> = ({ warning, regionName, onClick }) => {
    ...
    if (warning.level === 'emergency') {
        ...
        Icon = AlertOctagon;
        label = 'WARNING';
    } else if (warning.level === 'cleared') {
        bgClass = 'bg-gray-100'; // Muted Grey
    ...
    // Contextual Icon Override based on title keywords (Only if not cleared)
    if (warning.level !== 'cleared') {
    ...
    return (
        <Card 
            ...
            className={`min-w-[200px] snap-center !p-5 !rounded-none !shadow-none border-2 relative overflow-hidden flex flex-col justify-between cursor-pointer hover:brightness-105 active:scale-[0.98] transition-all group ${bgClass} ${borderClass}`}
        >
             {/* Hazard Pattern Overlay (Only for active warnings) */}
             {warning.level !== 'cleared' && (
             ...
             <div className="flex justify-between items-start mb-4 relative z-10">
                 <div className="flex flex-col items-start gap-1">
                    <div className={`text-[10px] font-black uppercase tracking-widest ${textClass} ${warning.level === 'cleared' ? 'bg-gray-200' : 'bg-black/10'} px-2 py-0.5`}>
                        {regionName}
                    </div>
                 ...
                 <div className="flex gap-2 items-start">
                    ...
                    <div className={`border-2 border-current p-1 backdrop-blur-sm ${warning.level === 'cleared' ? 'bg-white' : 'bg-black/5'}`}>
             ...
             <div className="relative z-10">
                 <h4 className={`text-xl font-black font-sans uppercase leading-none tracking-tight ${textClass} drop-shadow-sm ${warning.level === 'cleared' ? 'line-through decoration-2 decoration-gray-300' : ''}`}>
                     {warning.title}
                 </h4>
                 <div className={`text-[9px] mt-2 font-mono uppercase opacity-70 ${textClass} flex items-center gap-1`}>
                     {warning.level === 'cleared' ? 'Warning Lifted' : 'JMA Official Feed'}
                 </div>
             </div>
        </Card>
    );
